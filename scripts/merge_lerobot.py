...
def load_episode_list_file(folder):
    """
    加载meta/episode_list.txt，返回条目列表；如果不存在则返回空列表
    """
    episode_list_path = Path(folder) / "meta" / "episode_list.txt"
    entries = []
    if episode_list_path.exists():
        try:
            ...
            print(f"Warning: 读取 {episode_list_path} 失败: {e}")
    else:
        print(f"Warning: {episode_list_path} 不存在，无法加载episode_list")
    return entries


def save_episode_list(entries, output_folder):
    """
    将合并后的episode_list写入输出数据集的meta目录
    """
    if not entries:
        print("Warning: 没有可写入的episode_list条目")
        return
    output_path = Path(output_folder) / "meta" / "episode_list.txt"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(output_path, "w", encoding="utf-8") as f:
        for entry in entries:
            f.write(f"{entry}\n")
    print(f"Saved merged episode list to {output_path}")
...
def merge_datasets(
    ...
    for folder in source_folders:
        try:
            ...
            episode_list_entries = load_episode_list_file(folder)
            ...
            for episode in episodes:
                ...
                # 维护episode_list
                if episode_list_entries:
                    if episode_list_index < len(episode_list_entries):
                        merged_episode_list.append(episode_list_entries[episode_list_index])
                        episode_list_index += 1
                    else:
                        fallback_entry = f"{Path(folder).name}_episode_{old_index:06d}"
                        print(
                            f"Warning: {folder} 的 episode_list 条目不足，使用回退名称 {fallback_entry}"
                        )
                        merged_episode_list.append(fallback_entry)
                else:
                    merged_episode_list.append(f"{Path(folder).name}_episode_{old_index:06d}")

            # 使用收集的唯一任务列表替换之前的任务处理逻辑
            all_tasks = all_unique_tasks

            if episode_list_entries and episode_list_index < len(episode_list_entries):
                remaining = len(episode_list_entries) - episode_list_index
                print(
                    f"Warning: {folder} 的 episode_list.txt 中有 {remaining} 条记录未被使用"
                )
    ...
    save_episode_list(merged_episode_list, output_folder)
...
def get_arguments():
    ...
    parser.add_argument('--fps', action='store', type=int, help='fps', default=30, required=False)
    parser.add_argument('--dim', action='store', type=int, help='dim', default=14, required=False)
...
if __name__ == "__main__":
    args = get_arguments()
    merge_datasets(args.datasetDir, args.targetDir, max_dim=args.dim, default_fps=args.fps)